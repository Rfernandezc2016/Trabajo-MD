...
table(setas_train$Letalidad[k50$medoid])
```
...
summary(pca.50)
plot(pca.50$x,col=setas_train$Letalidad[k50$medoid]+1,pch=19)
```
...
Observamos algo bastante parecido a lo obtenido en el análisis anterior, dos zonas bastante bien diferenciadas y otra zona donde coinciden las setas venenosas y las comestibles. Luego podemos llegar a suponer que el número de cluster necesarios en un futuro análisis cluster serían 5, para intentar clasificar lo mejor posible la nube de puntos de $PC1\in(-1,2)$ y $PC2\in(-1,0.5)$. 



Quizás no sean necesarios 50 medoides para realizar el análisis y podríamos obtener un resultado parecido con otra cantidad. Probamos con una cantidad muy pequeña.
...
while(i<=10){
  ...
  #Análsiis de componentes principales
  pca.traink=prcomp(distancia_dice[,k$medoid])
  summary(pca.traink)
  plot(pca.traink$x,col=setas_train$Letalidad+1,pch=19,main=paste("Número de medoides ",as.character(i)))
i=i+1
}
```
...
Ahora vamos a probar a hacerlo con una cantidad distintas de medoides, desde 10 medoides hasta 250 aumentando de diez en diez.
...
while(i<=250){
  ...
  #Análsiis de componentes principales
  pca.traink=prcomp(distancia_dice[,k$medoid])
  plot(pca.traink,main=paste("Número de medoides ",as.character(i)))
  plot(pca.traink$x,col=setas_train$Letalidad+1,pch=19,main=paste("Número de medoides ",as.character(i)))
i=i+10
}
```

Los resultados son bastantes parecidos cuando cogemos una cantidad de medoides entre 10 y 250, pero debemos fijarnos en la proporción de la varianza acumulada explicada por las dos primeras componentes y vemos que obtenemos la más alta con 60 medoides
Por de ello, nos quedaremos con la distancia de todas las observaciones a los 60 medoides, para tener más información.
...
#Ajustamos el modelo
setas.svm.lineal=svm(setas50_train,setas_train[,1],type="C-classification",kernel="linear",cost=mean(cost_est),scale=F)

#Tabla comparativa
prediccion_svm.lineal=predict(setas.svm.lineal,setas50_test)
...
setas_train=setas_train[,-(23:28)]
setas_train=setas_train[,-(24:25)]
```