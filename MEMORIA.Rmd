...
```
Dos cluster agrupan exclusivamente setas venenosas, los otros 6 cluster hay setas de ambas toxicidad.
...
La tabla nos dice que el modelo predice a la perfección los datos, podría estar sobreajustado. Las medidas de precisión no van a aportar nada que no sepamos con mirar simplemente la tabla.
...
```
El mejor modelo para aplicar a nuestros datos es el SVM con el kernel radial, pues predice a la perfección los datos tanto los de entrenamiento como los de test, pero como hemos dicho antes, podría estar sobreajustado, por ello nos quedmos con el modelo k-NN.
...
En la gráfica que enfrenta las dos primeras componentes principales vemos que las setas venenosas están ubicadas en $P1\in (-2,-1)\cup(2,3)$ aunque hay unas pocas por medio de las comestibles.
...
La predicción con el 1-vecino más próximo, se equivoca en dos setas. Vamos a realizar un análisis con distintas medidas de precisión.
...
Observamos que al ir aumentando el número de k-vecinos el error va aumentando pero sigue siendo muy pequeño, el mayor error cometido con el número de k-vecinos probado es de 4%, pero solo se equivoca en una seta, el error es tan "grande" a comparación de otro análisis por la dimensión de nuestros datos.
...
El error que se comete es 12%, pero solo se equivoca en dos setas, así que el modelo predice bastante bien.
...
La predicción se equivoca clasificando 4 setas. Analizamos la precisión de los bosques aleatorios con las medidas de precisión. 
...
La tabla nos dice que el modelo predice casi la perfección los datos pero el modelo está sobre ajustado, pues requiere de 144 vectores soporte cuando el número de observaciones es 144. Las medidas de precisión van a aportar el error cometido.
...
Podemos ver que respecto al kernel radial, las predicciones son parecidas pero necesita de menos vectores soportes para ajustar el modelo.
...
```
El mejor modelo para aplicar a nuestros datos es puede ser el k-NN con el vecino más próximo o el SVM con el kernel lineal, pues predice bastante bien los datos tanto los de entrenamiento como los de test, solo se comete un error del 4%. La conclusión a la que hemos llegado tras finalizar el análisis de los datos difiere un poco del resto de conclusiones.
