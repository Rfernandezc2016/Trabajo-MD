...
Si usamos el 1 vecino más cercano, podemos ver en sus medidas de precisión que el ajueste es bastante bueno, es decir, que predice la toxicidad de las setas con un grado de acierto muy alto, solo comete un error del 0.3%.
...
#Error de predicción
table(pred=predict(arbol,type="class"), obs=setas50_train.respuesta$Venenosas)
prediccion.arbol=round(predict(arbol,as.data.frame(setas50_test))[,2])
...
prediccion.rf=predict(setas.rf, setas50_test)
#Tabla comparativa
table(prediccion.rf,as.factor(setas_test))
```

Las medidas de precisión del modelo de bosques aleatorios son:
```{r medidas de presicion bosques aleatorios,echo=TRUE}
# Exactitud/Error
...
#Ajustamos el modelo
setas.svm=svm(svm_x,svm_y,type="C-classification",kernel="radial",gamma=mean(gamma_est),cost=mean(cost_est),scale=F)
#Predecimos
...
# Medidas de precisión
accuracy = sum(prediccion_svm == setas50_train.respuesta$Venenosas)/nrow(setas50_train.respuesta)
error = 1-accuracy

# Acierto sobre el total de las setas VENENOSAS, sensitivity o recall
sensitivity = sum((prediccion_svm == setas50_train.respuesta$Venenosas) & (setas50_train.respuesta$Venenosas == 1)) / sum(setas50_train.respuesta$Venenosas == 1)
recall = sensitivity

# Acierto sobre el total de las setas COMESTIBLES
specificity =  sum(prediccion_svm == setas50_train.respuesta$Venenosas & setas50_train.respuesta$Venenosas == 0) / sum(setas50_train.respuesta$Venenosas == 0)

# Acierto cuando el predicho es VENENOSAS
precision = sum((prediccion_svm== setas50_train.respuesta$Venenosas) & (prediccion_svm == 1))/ sum(prediccion_svm == 1)

# Acierto cuando el predicho es COMESTIBLE
npv = sum(prediccion_svm== setas50_train.respuesta$Venenosas & prediccion_svm == 0) / sum(prediccion_svm == 0)
...
parametros=tune.svm(setas50_validacion, setas_validacion[,1], gamma=seq(5,8,by=0.5), cost=seq(10,50,by=10))
attributes(parametros)

# Calculamos el modelo con estos parámetros
setas.svm_parametros=svm(svm_x, svm_y, type="C-classification", kernel="radial", gamma= parametros$best.parameters[1], cost= parametros$best.parameters[2], scale=F)
