...
par(mfrow=c(1,2))
plot(pca.50$x)
points(pca.50$x[comestible,1:2],col="red",pch=19)
points(pca.50$x[no.comestibles,1:2],col="black",pch=19)
plot(pca.50$x)
points(pca.50$x[no.comestibles,1:2],col="black",pch=19)
points(pca.50$x[comestible,1:2],col="red",pch=19)
```
...
n=dim(pca.train$x)[1]
#Variabilidad de todos los datos, es decir, todos los datos como un único cluster
SSW[1] <- (n - 1) * sum(apply(pca.train$x,2,var)) 

#Variabilidad de cada modelo, desde 2 clusters hasta 15 clusters
for (i in 2:15) SSW[i] <- sum(kmeans(pca.train$x,centers=i,nstart=25)$withinss)
...
#Análisis cluster
clusters.datos=kmeans(pca.train$x,centers=8,nstart=25)
