...
while(i<10){
  ...
  table(setas_train$Letalidad[k$medoid])
  ...
  if(proporcion[2]==0){
    #Proporción de la varianza acumulada explicada por la primers componente principal
...
Ahora vamos a probar a hacerlo con una cantidad distintas de medoides, desde 10 medoides hasta 250 aumentando de diez en diez. Nos quedaremos con aquella que tenga mayor proporcion acumulada pero sin sacrificar información sobre los datos.
...
Los resultados son bastantes parecidos cuando cogemos una cantidad de medoides entre 10 y 250, pero debemos fijarnos en la proporción de la varianza acumulada explicada por las dos primeras componentes y vemos que obtenemos la más alta con 20 medoides, a pesar de ello, nos quedaremos con la distancia de todas las observaciones a los 60 medoides, la segunda proporcion acumulada más alta, para tener más información.
...
Observamos algo bastante parecido a lo obtenido en el análisis anterior, dos zonas bastante bien diferenciadas y otra zona donde coinciden las setas venenosas y las comestibles. Luego podemos llegar a suponer que el número de cluster necesarios en un futuro análisis cluster serían por lo menos 4 o 5, para intentar clasificar lo mejor posible la nube de puntos de $PC1\in(-1,2)$ y $PC2\in(-1,0.5)$. 
...
n=dim(prcomp(setas60_train)$x)[1]
#Variabilidad de todos los datos, es decir, todos los datos como un único cluster
SSW[1] <- (n - 1) * sum(apply(prcomp(setas60_train)$x,2,var)) 

#Variabilidad de cada modelo, desde 2 clusters hasta 15 clusters
for (i in 2:15) SSW[i] <- sum(kmeans(prcomp(setas60_train)$x,centers=i,nstart=25)$withinss)
...
for (i in 1:round(k)){
  prediccion_knn=knn(setas_train[k60$medoid,],setas_test,cl=setas_train$Letalidad[k60$medoid],k=i)
...
colnames(setas60_test.data)=colnames(setas60_train.respuesta)[2:61]
...
El error que se comete es inferior que en el caso del modelo k-NN con el vecino más próximo, pero sigue siendo un error aceptable para el modelo, por lo que podemos determinar que el árbol de decisión predice casi todas las setas correctamente.
...
El error que se comete mejora respecto a los dos algoritmo anteriores.
...
```{r svm con ajuste de parametros,echo=FALSE}
#Ajustamos los parámtros
parametros=tune.svm(setas60_validacion, setas_validacion[,1], gamma=seq(5,8.5,by=0.5), cost=seq(10,30,by=10))
...
Cuando realizamos el análisis descriptivo, ya observamos que la variable **Olor** es bastante determinante a la hora de clasificar las setas según su toxicidad, pero ello nos planteamos realizar todo el análisis anterior quitando esta variable.
...
Para seleccionar el número de medoides adecuado, hacemos cross-validation pero como sabemos que quizas el número de medoides adecuado está cuando son más de 10, probamos directamente. Nos quedaremos con aquella que tenga mayor proporcion acumulada pero sin sacrificar información sobre los datos.
...
while(i<=250){
  ...
  #Proporción de los medoides:
  table(setas_train.sin.olor$Letalidad[k$medoid])
...
Observamos una gran maraña de puntos donde solo podemos ver dos grupos bien diferenciados del resto.
...
#Números de clusters
SSW <- vector(mode = "numeric", length = 15)
n=dim(prcomp(setas60_train.sin.olor)$x)[1]
#Variabilidad de todos los datos, es decir, todos los datos como un único cluster
SSW[1] <- (n - 1) * sum(apply(prcomp(setas60_train.sin.olor)$x,2,var)) 

#Variabilidad de cada modelo, desde 2 clusters hasta 15 clusters
for (i in 2:15) SSW[i] <- sum(kmeans(prcomp(setas60_train.sin.olor)$x,centers=i,nstart=25)$withinss)
