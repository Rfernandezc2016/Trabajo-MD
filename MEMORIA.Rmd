...
Para poder tranformar las variables en factores, lo hacemos aplicando el comando **as.factor()** a todas las variables.
...
Como hemos dicho, la medidas de similutd no es la correcta, por ello descartamos esta opción.
...
La tabla nos dice que el modelo predice a la perfección los datos, utiliza 977. Las medidas de precisión no van a aportar nada que no sepamos con mirar simplemente la tabla.
...
medidas.precision
```
El mejor modelo para aplicar a nuestros datos es el SVM con el kernel radial, pues predice a la perfección los datos tanto los de entrenamiento como los de test, pero el modelo de bosques aleatorios también es un buen modelo para aplicar ya que tiene una sensibilidad del 100%.
...
La predicción se equivoca clasificando ciertas setas, empeora respecto la predicción con la variable **Olor**. Analizamos la precisión de los bosques aleatorios con las medidas de precisión. 
...
```
El mejor modelo para aplicar a nuestros datos es el SVM con el kernel radial, pues predice a la perfección los datos tanto los de entrenamiento como los de test, pero como hemos dicho antes, podría estar sobreajustado, por ello nos quedamos con el modelo k-NN.
...
Con solo las dos primeras componentes principales explicamos un 73% de la variabilidad de los datos, es decir un nivel parecido que teníamos en análsis anteriores.

En la gráfica que enfrenta las dos primeras componentes principales vemos que las setas venenosas están ubicadas en por grupos, mientras que hay algunas comestibles más esparcidas.


Las variables no necesitan ninguna modificación adicional, pues ya las hemos codificado anteriormente. Lo único que transformamos son nuestro datos. Calculamos la distancia dice en el conjunto de datos de test y validación, esto permite utilizar la distancia euclínea en los algoritmos posteriores.
...
setas.dice_test.olor=setas.dice_test.olor[-c(1:dim(setas_train.olor)),c(1:dim(setas_train.olor))]
...
setas.dice_validacion.olor=setas.dice_validacion.olor[-c(1:dim(setas_train.olor)),c(1:dim(setas_train.olor))]
...
La predicción con el 1-vecino más próximo, se equivoca muy poco. Aplicamos un análisis con distintas medidas de precisión.
...
El error que se comete es del 10%, pero solo se equivoca en 5 setas, así que el modelo predice bastante bien.
...
La predicción se equivoca clasificando ciertas setas. Analizamos la precisión de los bosques aleatorios con las medidas de precisión. 
...
Vemos que el modelo k-NN es el mejor pues aunque hayamos ido cambiando los datos con los que hemos ajustado el modelo, el error cometido todos los análisis no supera el 2%, siempre que lo hagamos con el vecino más próximo.