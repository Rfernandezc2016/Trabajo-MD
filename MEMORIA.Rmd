...
summary(datos.setas)
```
Una de las variables, **Tipo_v** (tipo de volva) con dos niveles distintos, parcial (p) y universal (u), podemos eliminarla de nuestros datos pues todo la frecuencia solo está en uno de los niveles, luego no aporta información alguna para determinar si es venenosa o comestible. La variable **Tipo_v** es la número 17.

```{r eliminar variable,echo=TRUE}
datos.setas=datos.setas[,-17]
```

Quizás trabajar con datos que todas sus variables sean cualitativas no sea lo más adecuado, por ello nos planteamos codificar las variables de forma que sean binarias, para ello cada subnivel de las variables originales se va a convertir en una nueva variable binaria, de forma que será 0 si no se presenta ese subnivel en la variable original y 1 si si lo hace.

 + La variable respuesta, establecemos que 1 es si la seta es venenosa y 0 si es comestible. 
 + La variable **Escamas**, la codificamos como 1 si la seta presenta escamas y 0 si no tiene escamas.
 + La variable **Tamaño_L**, establecemos que es 1 si el tamaño de las láminas de la seta es ancho y 0 si es estrecho. 
 + La variable **Forma_T**, la establecemos como 1 si es agrandado y 0 si es afilado.
 + La variable **Fijacion_L** tendría solo dos subniveles pues los subniveles descendente y entallado no aparecen en los datos, por ello establecemos que 1 es si la variable **Fijacion_L** es adjunto y 0 si es libre.
 + La variable **Espacio_L** también tiene dos subniveles pues el subnivel distante tampoco aparece, por ello la codificamos como 1 si la variable **Espacio_L** es cerca y 0 si es atestado.
...
### Letalidad:
La variable **Letalidad** tiene dos niveles distintos, venenosa (1) o comestible (0), la frecuencia relativa correspondiente a nuestros datos es:
...
Vamos a comprobar si existe alguna correlación entre las distintas variables, para ello usamos los datos sin codificar de forma binaria.
...
Podemos observar que la variable **Olor** se distribuye entre los distintos tipos de olor de forma exclusiva, es decir, todas las setas que presentan un olor del tipo *creosota, asqueroso, mohoso, acre, picante o pescado* son venenosas mientras que si presentan un olor del tipo *almendra o anís * son comestibles, mientras que si es *ninguno* el olor tienes una probabilidad del 96.6% de que sea comestible. 
...
```
Entonces podríamos cambiar la codificación hecha anteriormente y tener solo una variable codificada de forma binaria de tal forma que para los olores que producen que la seta sea venenosa darles el valor 1 y para los que no, darles el valor 0, pero las setas con olor *ninguno* no podríamos asignarlas a ninguno de esos valores porque hay setas venenosas y comestibles y por tanto tendríamos que añadir un nuevo valor para ellas.
...
Para calcular **a** como buscamos que ambos valores de la variable de cada observaciones sean 1 y sumamos el número de veces que al multiplicar cada valor con el otro sea 1, es decir, hacemos el sumatorio del producto escalar de los dos vectores de las observaciones. 

Como calcular **b, c** supone más trabajo, calculamos **d** y lo hacemos de una forma parecida a **a**, pero le restamos cada valor de la variable a la unidad para después hacer el sumatorio del producto escalar, la razón de hacer esto es convertir aquellas variables que tengan valor 1 en 0 para que no nos lleve a error al calcular el número de veces que ambas observaciones son 0, ya que en el caso de que una de las variables binarias es 1 en una observaciones y 0 en la otra también obtendríamos ese resultado.
...
En el gráfico podemos observar que a partir de aplicar 5 clusters a nuestros datos la suma de los cuadrados se reduce en menor medida, por ello realizamos el análisis con ese número de clusters.
...
Tras haber analizado los resultados que obtenemos al aplicar los modelos a nuestros datos, nos planteamos ciertas actualizaciones no a los modelos sino a los datos.

## Datos sin variable **Olor**

Cuando realizamos el análisis descriptivo, ya observamos que la variable **Olor** es bastante determinante a la hora de clasificar las setas según su toxicidad, pero ello nos planteamos realizar todo el análisis quitando esta variable de los datos. Las variables asociadas al olor se sitúan entre las columnas 23-31.
...
Vemos que tras realizar el análisis clúster con 8 grupos, notamos un empeoramiento de la predicción a pesar que ahora tenemos tres cluster bien diferenciados, por lo que nos quedamos con el análsis clúster con $k=5$.
...
Cuando realizamos el análisis descriptivo, ya observamos que la variable **Olor** es bastante determinante a la hora de clasificar las setas según su toxicidad, por ello nos planteamos realizar todo el análisis dejando solo las setas cuyo olor sea ninguno.
...
De las setas comestibles elegimos 120 setas de forma aleatoria para que se unan a las setas venenosas que tienen olor ninguno y usar esos datos para realizar el análisis.
...
Transformamos nuestro datos calculándo la medida Dice en el conjunto de datos de test y validación, esto permite utilizar la distancia euclínea en los algoritmos posteriores.
...
Seguimos sin poder separar las setas que no estaban en su debido cluster en el análisis con 6 clusters, además de que ha aparecido otro cluster con setas mezcladas.
...
El mejor modelo para aplicar a nuestros datos puede ser el k-NN con el vecino más próximo o los bosques aleatorios, pues ambos modelos predicen bastante bien los datos tanto los de entrenamiento como los de test, solo se comete un error del 4,1%. La conclusión a la que hemos llegado tras finalizar el análisis de los datos difiere un poco del resto de conclusiones. Aunque, al ser pocos datos, puede que no sea una muestra lo suficientemente representativa (a pesar de haber seleccionado las setas venenosas de forma aleatoria) y por lo tanto las conclusiones pueden ser erróneas.
