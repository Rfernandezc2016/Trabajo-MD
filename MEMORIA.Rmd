...
Dada la naturaleza de nuestros datos, no nos es posible usar las distancias euclideas como medida para comparar como de cerca están dos observaciones, por ellos debemos usar otras medidas de similitud. Para ello, usamos la siguiente tabla:
...
Hacemos un intento calcular nuestra propia medida de similitud, pero vemos que el proceso es muy largo debido a la dimensión de la matriz resultante, por ellos buscamos alguna función predeterminada en R que sea capaz de hacerla.
...
La función que utilizaremos es *distance()* de la librería **_philentropy_**, a la cual debemos introducirle como parámetros la matriz con nuestros datos codificados y el método por el cual queremos calcular la distancia, en nuestro caso, utilizamos *method="dice"*. Calcula exactamente:
$$d = \dfrac{\sum(P_i − Q_i)^2}{(\sum P_i^2 + \sum Q_i^2)}$$
```{r distancias,echo=TRUE}
#Distancia Dice
distancia_dice=distance(setas_train[,2:117],method="dice")

dice=designdist(setas_train[1:10,2:117],"(2*a)/(2*a+b+c)",abcd=TRUE)
```
...
par(mfrow=c(1,2))
plot(pca.50$x)
points(pca.50$x[comestible,1:2],col="red",lwd=2)
points(pca.50$x[no.comestibles,1:2],col="black",lwd=2)
plot(pca.50$x)
points(pca.50$x[no.comestibles,1:2],col="black",lwd=2)
points(pca.50$x[comestible,1:2],col="red",lwd=2)
...
##  k-NN
```{r knn}
trainTest=rbind(setas_train[k50$medoid,],setas_test)
distancia_trainTest=distance(trainTest,method="dice")
distancia_trainTest=distancia_trainTest[1:50,-c(1:50)]
