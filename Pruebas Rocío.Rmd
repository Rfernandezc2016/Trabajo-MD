...
pairs.panels(datos.setas_train[,1:5],method = "pearson",hist.col = "#00AFBB",density = TRUE,ellipses = TRUE)
...
for (i in 1:(dim(setas)[1]-1)){
  for (j in (i+1):(dim(setas)[1])) {
    ...
    distancia_hamman=dist.dice(a,b,c,d,distancia_hamman,i,j)
    distancia_pearson=dist.dice(a,b,c,d,distancia_pearson,i,j)
  }
}
```