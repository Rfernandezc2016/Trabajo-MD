...
```{r PCA1}
#con esto calculo los medoides pero no se que hacer con ellos
library(kmed)
k=fastkmed(dice_train,ncluster=30)

#revisar esto puede estar mal 
library(kmed)
k=fastkmed(distancia_dice,ncluster=30)
matriz_train_k=rbind(setas_train[,2:117],k$centers)
matriz_distancia_k=distance(matriz_train_k,method="dice")
dim(setas_train)
dim(matriz_distancia_k[3,])
pr=matriz_distancia_k[dim(setas_train)[1]:dim(matriz_distancia_k)[1],1:dim(setas_train)[1]]
pca_prueba=prcomp(matriz_distancia_k[dim(setas_train)[1]:dim(matriz_distancia_k)[1],1:dim(setas_train)[1]])
plot(pca_prueba)
summary(pca_prueba)
nuevos.datos.pca.prueba=pca_prueba$x[,1:4]
dim(nuevos.datos.pca.prueba)
dim(nuevos.datos.pca)
pca.venenosas.prueba=which(setas_train[,1]==1)
pca.novenenosas.prueba=which(setas_train[,1]==0)

plot(nuevos.datos.pca.prueba)
points(nuevos.datos.pca.prueba[pca.venenosas.prueba,],col="aquamarine",lwd=2)
points(nuevos.datos.pca.prueba[pca.novenenosas.prueba,],col="purple",lwd=2)

plot(nuevos.datos.pca.prueba)
points(nuevos.datos.pca.prueba[pca.novenenosas.prueba,],col="purple",lwd=2)
points(nuevos.datos.pca[pca.venenosas.prueba,],col="aquamarine",lwd=2)


pairs(nuevos.datos.pca.prueba[,1:4])
```
...
fin - inicio
#Parece que funciona
dice_train=distance(setas_train[,2:117],method="dice")


distancia_jaccard=diag(dim(setas)[1])
inicio = Sys.time()
distancia_jaccard=distance(setas,method="jaccard")
fin = Sys.time()
fin - inicio
